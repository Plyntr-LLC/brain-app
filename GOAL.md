...
- Mac one-file installer: `npm run pack:mac` writes a signed arm64 dmg. Latest packed: **0.1.62**. Developer ID Application: Plyntr LLC. Setup installs Cloudflare Tunnel on every path, and chat stays closed until Git, Tunnel, and the chosen CLI are ready. An agency owner chats on the draft until the GitHub app and Brain Bridge have both passed. Back-to-back Thinking rows fold into one entry. A visible card starts a new one. A website link opens in the browser, and Settings shows the local roster before the network. A missing Brain Bridge answer does not stop GitHub sync. All repositories still does. Adding a company asks for Starter, Standard, or Growth, and superadmin can change that plan later. Starter stops at the owner and two other people.
