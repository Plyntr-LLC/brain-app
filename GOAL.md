...
- Settings: one company at the top (You are working on this company). Switch brain lists local brains on this Mac without waiting on plyntr:companies. Joe superadmin sees every local folder plus signed-in brains. Other people see signed-in brains that have a local folder. Switch retargets Agency Brain and Plyntr project sync. Add a company brain: paste the Ads2AI code. Setup if GitHub is missing, clone if the repo exists. Add users and Catalog school stay collapsed.
...
- Mac one-file installer: `npm run pack:mac` writes a signed arm64 dmg. Latest packed: **0.1.72**. Developer ID Application: Plyntr LLC. The title shows only the account signed into the open brain. On chat, Model and Effort stay in the right sidebar only, and the typing line stays on screen. Setup installs Cloudflare Tunnel on every path, and chat stays closed until Git, Tunnel, and the chosen CLI are ready. An agency owner chats on the draft until the GitHub app and Brain Bridge have both passed. Back-to-back Thinking rows fold into one entry. A visible card starts a new one. A website link opens in the browser, and Settings shows the local roster before the network. A missing Brain Bridge answer does not stop GitHub sync. All repositories still does. Adding a company asks for Starter, Standard, or Growth, and superadmin can change that plan later. Starter stops at the owner and two other people.
...
- [x] 2026-09-26 Joe: After 0.1.71, Settings told him to sign in for project-only people even though GitHub apps were already on the repo, Email me a project-only code did nothing, first-chat still asked about the business after AGENTS.md was filled, in-app refresh flashed the start page, and logging in on a new Mac stopped the old Mac syncing. Claude Chat/picker was him not logged in. Modest setup polish only. Do not change Claude Chat. Done: 2026-09-26. Evidence: Brain.app 0.1.72. Opus 5.5 APPROVE after open-list re-review. Project-only is folded closed and the email code reports failure. First chat skips a filled AGENTS.md and team/project seats. Refresh shows Opening… instead of the start page. Worker: a second Mac no longer kills the first seat; removing a person still cuts off every Mac. An old Mac that already lost its token needs a new code.

- [x] 2026-09-26 Joe: Brand-new Mac still stuck on Cloudflare Tunnel. Terminal opened, Homebrew errored, Process completed, tunnel missing. Mac setup now downloads Cloudflare’s official binary into Brain’s own folder. No Homebrew, no Terminal, no password. Phone uses the same binary. Windows still finds a winget install (`where` / `.exe`). Done: 2026-09-26. Evidence: Brain.app 0.1.71. Opus 5.5 medium APPROVE after open-list re-review.

- [x] 2026-09-26 Joe: Opus 5.5 high REJECT on first-run setup copy. Every setup screen now uses short plain lines, buttons match what happens next, Recheck no longer blames a phantom installer while setup is running, and “not signed in” opens the sign-in screen. Done: 2026-09-26. Evidence: Brain.app 0.1.69. Approved by Opus 5.5 high (copy review), 2026-09-26.

- [x] 2026-09-26 Joe: Brand-new Mac stuck on Cloudflare Tunnel (silent brew, Recheck blamed a missing installer), setup showed Electron “Error invoking…” text, and dead Model/Effort sat on setup pages. Cloudflare Tunnel now opens Terminal the same way Homebrew does and waits until it is installed. Recheck names the missing tool. Setup errors drop the IPC wrapper. Setup Model/Effort is hidden; Chat pickers stay. Settings type flatten from the same day stays. Done: 2026-09-26. Evidence: Brain.app 0.1.68.

- [x] 2026-09-26 Joe: Settings on 0.1.66 had no Switch brain. Joe had one keyed brain (rose-wine) and agency-brain on disk with no brainId, never remembered as keyless, so `allowedFolders` gave one entry. Now `shell:view` also returns `local` (brains.json folders on this Mac). With `local`, `allowedFolders` lists folders by path: for Joe super admin every local folder plus his signed-in ones, for anyone else only signed-in brains that have a local folder. The Settings select maps each entry to its folder path through `switchOption`. Switching a folder with no brainId for Joe remembers it as keyless and switches by path. A keyed folder switches only by its own sign-in; Joe opening a keyed brain he is not signed into is refused and never recorded as keyless. A failed `plyntr:companies` shows the short offline note only for network errors; 401/403 go back to sign in (main throws `PLATFORM_AUTH` on 401/403 from the company list, company read, open company and create brain); other errors show a short message. Done: 2026-09-26. Evidence: Brain.app 0.1.67. The switcher lists local brains without waiting on plyntr:companies. Approved by Opus 5.5 medium, 2026-09-26. Check: `scripts/check-shell-switch.ts` items 9L and 13c (a real 401 and 403 sent through the handlers).
